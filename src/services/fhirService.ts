...
export class FHIRService {
  private static async makeRequest<T>(endpoint: string, searchParams?: Record<string, string | number>): Promise<T> {
  ...
  static async searchPatients(searchParams: PatientSearchParams = {}): Promise<FHIRBundle> {
    ...
    if (searchParams._sort) {
      params._sort = searchParams._sort;
    }
    if (searchParams._order) {
      params._order = searchParams._order;
    }
...
export class FHIRUtils {
  ...
  static getPatientMRN(patient: any): string {
    ...
    const mrn = patient.identifier.find((id: any) => id.type?.coding?.[0]?.code === "MR");
  ...
  static getPatientPhone(patient: any): string {
    ...
    const phone = patient.telecom.find((contact: any) => contact.system === "phone");
  ...
  static getPatientEmail(patient: any): string {
    ...
    const email = patient.telecom.find((contact: any) => contact.system === "email");
